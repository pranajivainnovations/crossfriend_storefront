  ...
  return (
    <main className="min-h-screen overflow-x-hidden bg-gradient-to-b from-[#f9f6ff] via-white to-[#fcfaff]">
      ...
      <div className="content-container flex flex-col items-center gap-4 py-16">
        <div className="h-10 w-3/4 max-w-2xl animate-pulse rounded-lg bg-cf-purple-100" />
        <div className="h-10 w-1/2 max-w-xl animate-pulse rounded-lg bg-cf-purple-100" />
        <div className="mt-2 h-5 w-2/3 max-w-lg animate-pulse rounded bg-cf-purple-50" />
      </div>
        ...
        <div className="mx-auto max-w-3xl rounded-2xl border border-cf-purple-100 bg-white p-6 shadow-sm">
          ...
          <div className="mt-6 grid grid-cols-2 gap-3 sm:grid-cols-4">
            {[0, 1, 2, 3, 4, 5, 6, 7].map((i) => (
              <div
                key={i}
                className="h-11 animate-pulse rounded-lg bg-cf-purple-50"
              />
            ))}
          </div>

          <div className="mt-6 h-12 w-full animate-pulse rounded-xl bg-cf-purple-100" />
        </div>
      </div>
    </main>
  )
